...
$config = require_once($_SERVER['DOCUMENT_ROOT'].'/plugins/extregister/config/config.php');

if (!isset($_SESSION['captcha_timestamp']))
{
	$_SESSION['captcha_timestamp'] = microtime(true);
}
elseif ($_SESSION['captcha_timestamp'] > (microtime(true)-$config['timestamp_protect']))
{
	exit('Captcha attacked');
}
$_SESSION['captcha_timestamp'] = microtime(true);
